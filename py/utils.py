...
def load_data_original(dataset_str, return_groundtruth=False):
    """Load data."""
    names = ['x', 'y', 'tx', 'ty', 'allx', 'ally', 'graph']
    objects = []
    for i in range(len(names)):
    ...
    if not return_groundtruth:
        return adj, features, y_train, y_val, y_test, train_mask, val_mask, test_mask
    else:
        return adj, features, y_train, y_val, y_test, train_mask, val_mask, test_mask, [np.argmax(labels, axis=1), graph]


def load_location_data(dataset_str):
    if not os.path.isfile('data/input_%s.pkl' % dataset_str):
        f = open('data/input_%s.txt' % dataset_str)
        feature2id, label2id = {}, {}
        N = 0
        for line in f:
            if line[0] == '#':
            ...
            for x in a[1:]:
                ...
                if token not in feature2id:
                    feature2id[token] = len(feature2id)
        f.close()
        #graph = {i:[] for i in range(N)}
        edgelist = []
        features = np.zeros((N, len(feature2id)))
        ...
        output = open('data/input_%s.pkl' % dataset_str, 'wb')
        pkl.dump([adj, features, labels, train_index, val_index, test_index, feature2id, label2id], output, protocol=4)
    else:
        f = open('data/input_%s.pkl' % dataset_str, 'rb')
        [adj, features, labels, train_index, val_index, test_index, feature2id, label2id] = pkl.load(f)

    y_train, y_val, y_test = labels[train_index], labels[val_index], labels[test_index]
    label_names = [k for (k,v) in sorted(label2id.items(), key=operator.itemgetter(1))]
    return adj, features, y_train, y_val, y_test, np.array(train_index), np.array(val_index), np.array(test_index), label_names
